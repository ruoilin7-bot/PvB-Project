--[[ 
🌱 Auto-Buy Plants Script
Made by Rioo (PvB Project)
--]]
...
-- Listener stok
UpdStock.OnClientEvent:Connect(function(itemName, stock)
    if AutoBuy[itemName] and stock and stock > 0 then
        local now = tick()
        if not LastBuyT[itemName] or now - LastBuyT[itemName] >= BUY_COOLDOWN then
            LastBuyT[itemName] = now
            print("🔄 Auto-Buy:", itemName)
